...
const CustomerPendingBooking = () => {
  const dispatch = useDispatch()

  const { bookings } = useSelector((state) => state.serviceBooking)

  const payment = bookings.filter((book) => book.payment_status === 'pending')
  ...
  return (
    <Container>
      <Row>
        <h3 className="maintitle">Customer Pending Booking Report</h3>
      </Row>
      <Row>
        <Card className="my-3">
          <Card.Body>
            <DataTable
              ...
              filename="CustomerPendingBookingReport"
            />
          </Card.Body>
        </Card>
      </Row>
...
export default CustomerPendingBooking
